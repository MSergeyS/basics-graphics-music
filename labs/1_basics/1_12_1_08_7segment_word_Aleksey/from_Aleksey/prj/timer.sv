...
(
   ...
   reg work; // работа таймера
    ...
    if (rst) begin// сброс
        work <= '0;
    end else if (start && !ignore_input)
        work <= 1'b1;
    ...
    always_ff @ (posedge clk or posedge rst)
        if (rst)
            cnt_500ms <= '0;
        else if (n_500ms_i | (start && !ignore_input))
            cnt_500ms <= 500 - 1'b1;
    ...
    always_ff @ (posedge clk or posedge rst)
	 ...
	 always_ff @ (posedge clk or posedge rst)
        if (rst) begin
		  ...
		  else if (n_5000ms_i & n_500ms_i & work)
            ignore_input <= 1; // Начинаем игнорировать ввод
		  else if (ignore_input && out_500ms_i)
            ignore_input <= 0; // Прекращаем игнорировать, когда счетчик достиг 0
		  
    assign out_500ms_i = (ignore_input & n_500ms_i) || (n_5000ms_i & n_500ms_i);
