...
(
    ...
    logic [10:0] cnt_bounch;
    wire n_500ms_i, ignore_input;

    always_ff @ (posedge clk or posedge rst)
        if (rst)
            cnt_bounch <= '0;
        else if (n_500ms_i) // уменьшаем счётчик на 1 каждые 0,5 с
            cnt_bounch <= cnt_bounch - 1'd1;
        else if (bounch && !ignore_input)  // считаем число изменений только если не игнорируем ввод
            cnt_bounch <= cnt_bounch + 1'd1;
